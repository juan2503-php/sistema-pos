...
export default function NewOrder() {
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      navigate('/waiter/orders'); // Ir a vista de órdenes activas
    } catch (error) {
      toast.error(error.response?.data?.error || 'Error al enviar la orden');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="w-full md:w-[380px] lg:w-[450px] bg-neutral-800 flex flex-col h-1/2 md:h-full z-20 shadow-[-5px_0_20px_rgba(0,0,0,0.2)] border-t md:border-t-0 border-neutral-700 min-h-0">
        ...
        <div className="p-4 bg-neutral-900 border-t border-neutral-700 space-y-4 shadow-[0_-5px_15px_rgba(0,0,0,0.2)]">
          ...
          <button
            ...
            className="w-full py-4 rounded-xl bg-primary hover:bg-primary/90 text-white font-bold text-lg shadow-[0_4px_0_0_rgba(29,78,216,1)] active:shadow-none active:translate-y-1 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:shadow-none disabled:translate-y-0 disabled:cursor-not-allowed uppercase tracking-wide"
          >
            {submitting ? 'Enviando...' : (
              <>
                Confirmar Orden <Send className="w-5 h-5 ml-1" />
              </>
            )}
          </button>
        </div>
